    ...
    return (
        <div className={`flex-1 h-full overflow-y-auto custom-scrollbar animate-fade-in ${styles.bg}`}>
            <div className="p-6 flex flex-col gap-8 min-h-min">
                ...
                {/* HEADER */}
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 shrink-0 pt-10 md:pt-0">
                    <div>
                    ...
                    <div className="flex flex-wrap gap-3 w-full md:w-auto">
                        <div className="relative w-full md:w-auto group">
                            ...
                            <input
                                ...
                                className={`pl-10 pr-4 py-2 rounded-xl outline-none w-full md:w-48 md:focus:w-64 transition-all border ${theme === 'dark' ? 'bg-black border-purple-500/40 text-white focus:border-purple-500 focus:shadow-[0_0_15px_rgba(168,85,247,0.4)]' : 'bg-white border-[#C9A87C]/50 text-[#4B3621] focus:border-[#654321]'}`}
                                value={searchTerm}
                        ...
                        <div className="flex gap-3 w-full md:w-auto">
                            <button
                                ...
                                disabled={refreshing}
                                className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2 rounded-xl font-bold transition disabled:opacity-50 border ${theme === 'dark' ? 'bg-black border-purple-500/30 text-gray-300 hover:border-purple-500 hover:text-white hover:shadow-[0_0_15px_rgba(168,85,247,0.3)]' : 'bg-white border-[#C9A87C]/50 text-[#4B3621] hover:border-[#654321]'}`}
                            >
                            ...
                            <button
                                ...
                                className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2 rounded-xl font-bold transition hover:scale-105 border ${theme === 'dark' ? 'bg-purple-600 text-white hover:bg-purple-500 shadow-[0_0_20px_rgba(168,85,247,0.5)] border-purple-400' : 'bg-[#F5F5DC] text-[#4B3621] border-[#654321]/30 shadow-lg hover:bg-[#F5F5DC]/80'}`}
                            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 shrink-0">
                    ...
                    <div className={getCardStyle(portfolioIsProfit ? 'profit' : 'loss')}>
                        ...
                        <div className="flex items-center gap-3 mt-4">
                            <span className={`text-xs font-black px-3 py-1 rounded border ${portfolioIsProfit ? (theme === 'dark' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/50' : 'bg-emerald-50 text-emerald-700 border-emerald-300') : (theme === 'dark' ? 'bg-red-500/10 text-red-400 border-red-500/50' : 'bg-red-50 text-red-700 border-red-300')}`}>
                ...
                <div className="flex-1 flex flex-col xl:flex-row gap-8 min-h-0">
                    ...
                    {/* ASSET TABLE */}
                    <div className={`${getCardStyle('purple')} flex-1 !p-0 overflow-hidden`}>

                        {/* TABLE HEADER */}
                        <div className={`grid grid-cols-12 gap-2 px-4 md:px-6 py-4 border-b text-[10px] font-black uppercase tracking-widest ${theme === 'dark' ? 'border-purple-500/30 bg-purple-900/10 text-purple-400' : 'border-[#C9A87C]/30 bg-[#F5F5DC]/50 text-[#654321]'}`}>
                            {/* Mobile: Asset(7) + P/L(5) | Desktop: all cols */}
                            <div className="col-span-7 md:col-span-3">Asset</div>
                            <div className="hidden md:block col-span-1 text-right">Date</div>
                            <div className="hidden md:block col-span-1 text-right">Qty</div>
                            <div className="hidden md:block col-span-1 text-right">Avg</div>
                            <div className="hidden md:block col-span-1 text-right">Current</div>
                            <div className="hidden md:block col-span-1 text-right">Cost</div>
                            <div className="hidden md:block col-span-1 text-right">Value</div>
                            <div className="col-span-5 md:col-span-2 text-right">P/L</div>
                            <div className="hidden md:block col-span-1 text-center">Act</div>
                        </div>
                        ...
                        <div className="overflow-x-auto">
                            {filteredInvestments.length === 0 ? (
                                ...
                                filteredInvestments.map((inv, index) => {
                                    ...
                                    return (
                                        <div key={inv.id || inv._id || index} className={`grid grid-cols-12 gap-2 px-4 md:px-6 py-4 items-center border-b transition group text-sm ${theme === 'dark' ? 'border-purple-500/10 hover:bg-purple-500/5' : 'border-[#C9A87C]/20 hover:bg-[#F5F5DC]/30'}`}>

                                            {/* Asset — col-span-7 mobile, col-span-3 desktop */}
                                            <div className="col-span-7 md:col-span-3 flex items-center gap-3 overflow-hidden">
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <span className={`text-[10px] font-mono tracking-wide ${theme === 'dark' ? 'text-gray-500' : 'text-[#654321]/60'}`}>{metaLine || '-'}</span>
                                                </div>
                                            </div>

                                            {/* Date — hidden mobile */}
                                            <div className={`hidden md:block col-span-1 font-mono text-xs text-right opacity-70 ${theme === 'dark' ? 'text-gray-500' : 'text-[#654321]/60'}`}>
                                                {inv.date || '-'}
                                            </div>

                                            {/* Qty — hidden mobile */}
                                            <div className={`hidden md:block col-span-1 text-right font-bold font-mono ${theme === 'dark' ? 'text-gray-300' : 'text-[#4B3621]'}`}>
                                                {(inv.quantity || 0).toLocaleString()}
                                            </div>

                                            {/* Avg buy — hidden mobile */}
                                            <div className={`hidden md:block col-span-1 text-right font-mono text-xs ${theme === 'dark' ? 'text-gray-400' : 'text-[#654321]/70'}`}>
                                                {currency}{formatIndianNumber(displayBuyPrice)}
                                            </div>

                                            {/* Current price — hidden mobile */}
                                            <div className={`hidden md:block col-span-1 text-right font-bold font-mono ${theme === 'dark' ? 'text-purple-300 drop-shadow-[0_0_5px_rgba(168,85,247,0.5)]' : 'text-[#4B3621]'}`}>
                                                {currency}{formatIndianNumber(displayCurrentPrice)}
                                            </div>

                                            {/* Cost — hidden mobile */}
                                            <div className={`hidden md:block col-span-1 text-right font-mono text-xs ${theme === 'dark' ? 'text-gray-500' : 'text-[#654321]/60'}`}>
                                                {currency}{formatIndianNumber(itemCost)}
                                            </div>

                                            {/* Value — hidden mobile */}
                                            <div className={`hidden md:block col-span-1 text-right font-black font-mono ${theme === 'dark' ? 'text-white drop-shadow-[0_0_5px_rgba(255,255,255,0.5)]' : 'text-[#4B3621]'}`}>
                                                {currency}{formatIndianNumber(itemValue)}
                                            </div>

                                            {/* P/L — col-span-5 mobile, col-span-2 desktop */}
                                            ...
                                            <div className="hidden md:flex col-span-1 justify-center gap-1 opacity-50 group-hover:opacity-100 transition">
                    ...
                    {/* ALLOCATION CHART */}
                    <div className={`${getCardStyle('purple')} w-full xl:w-80 shrink-0`}>

                    ...
                    <div className="overflow-visible">
                        {processedData.map((t, index) => (
                            <div key={t.id || t._id || index} className={`grid grid-cols-12 px-6 py-4 border-b transition items-center group ${theme === 'dark' ? 'border-emerald-500/10 hover:bg-emerald-500/5' : 'border-[#C9A87C]/20 hover:bg-[#F5F5DC]/30'}`}>
                                ...
                                <div className={`col-span-2 text-right font-bold flex justify-end items-center gap-3 text-sm ${theme === 'dark' ? 'text-emerald-400 drop-shadow-[0_0_5px_rgba(16,185,129,0.6)]' : 'text-[#4B3621]'}`}>
                                    <span className="whitespace-nowrap">+{currency}{formatIndianNumber(parseFloat(t.amount || 0))}</span>
                                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0">
                                        <button onClick={() => handleEdit(t, 'income')} className={`p-1.5 rounded-lg transition border ${theme === 'dark' ? 'bg-black border-emerald-500/30 text-emerald-500 hover:bg-emerald-500 hover:text-black shadow-[0_0_10px_rgba(16,185,129,0.3)]' : 'bg-white text-[#654321] border-[#654321]/30 hover:bg-[#F5F5DC]'}`}><Edit2 size={14} /></button>

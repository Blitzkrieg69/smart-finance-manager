...
const Dashboard = ({ transactions, budgets, openModal, currency }) => {
  ...
  const getInsightIcon = (type) => {
    switch (type) {
      case 'success': return <CheckCircle size={16} className="text-emerald-500 flex-shrink-0 mt-0.5" />
      case 'warning': return <AlertTriangle size={16} className="text-yellow-500 flex-shrink-0 mt-0.5" />
      case 'danger': return <AlertCircle size={16} className="text-red-500 flex-shrink-0 mt-0.5" />
      default: return <Info size={16} className="text-blue-500 flex-shrink-0 mt-0.5" />
    }
  }

  const getInsightBorderStyle = (type) => {
    switch (type) {
      case 'danger': return theme === 'dark'
        ? 'border-l-4 border-l-red-500 bg-red-500/5 border-white/10 hover:bg-red-500/10'
        : 'border-l-4 border-l-red-400 bg-red-50 border-[#654321]/10'
      case 'warning': return theme === 'dark'
        ? 'border-l-4 border-l-yellow-500 bg-yellow-500/5 border-white/10 hover:bg-yellow-500/10'
        : 'border-l-4 border-l-yellow-400 bg-yellow-50 border-[#654321]/10'
      case 'success': return theme === 'dark'
        ? 'border-l-4 border-l-emerald-500 bg-emerald-500/5 border-white/10 hover:bg-emerald-500/10'
        : 'border-l-4 border-l-emerald-400 bg-emerald-50 border-[#654321]/10'
      default: return theme === 'dark'
        ? 'border-l-4 border-l-blue-500 bg-blue-500/5 border-white/10 hover:bg-blue-500/10'
        : 'border-l-4 border-l-blue-400 bg-blue-50 border-[#654321]/10'
    }
  }

  const displayedInsights = showAllInsights ? allInsights : insights
    ...
    <div className={`flex flex-col gap-6 h-full w-full p-6 overflow-y-auto custom-scrollbar ${styles.bg}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className={getCardStyle('blue')}>
          <div className="flex justify-between items-center">
            <div>
              <p className={`text-xs font-bold uppercase tracking-wider mb-2 ${theme === 'dark' ? 'text-blue-400' : 'text-[#654321]/70'}`}>Balance</p>
        ...
        <div className={getCardStyle('green')}>
          <div className="flex justify-between items-center">
            <div>
              <p className={`text-xs font-bold uppercase tracking-wider mb-2 ${theme === 'dark' ? 'text-emerald-400' : 'text-[#654321]/70'}`}>Income</p>
        ...
        <div className={getCardStyle('red')}>
          <div className="flex justify-between items-center">
            <div>
              <p className={`text-xs font-bold uppercase tracking-wider mb-2 ${theme === 'dark' ? 'text-red-400' : 'text-[#654321]/70'}`}>Expenses</p>
      ...
      {healthScore ? (
          ...
          <div className="flex flex-col lg:flex-row items-center gap-8">
            <div className="relative flex-shrink-0">
              <svg className="w-40 h-40 transform -rotate-90">
                <circle cx="80" cy="80" r="70" stroke={theme === 'dark' ? '#1e293b' : '#D2B48C'} strokeWidth="12" fill="none" />
                <circle
                  cx="80" cy="80" r="70"
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className={`text-sm font-bold ${theme === 'dark' ? 'text-gray-500' : 'text-[#654321]/60'}`}>/ 100</span>
              </div>
            </div>

            <div className="flex-1 w-full">
              <div className="flex items-center gap-2 mb-6">
                <h2 className={`text-2xl font-black ${theme === 'dark' ? 'text-white' : 'text-[#4B3621]'}`}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {Object.entries(healthScore.breakdown).map(([key, value]) => (
                  <div key={key} className={`p-4 rounded-xl ${theme === 'dark' ? 'bg-white/5 border border-white/10' : 'bg-[#F5F5DC] border border-[#654321]/20'}`}>
                    <p className={`text-xs font-bold uppercase mb-2 ${theme === 'dark' ? 'text-gray-400' : 'text-[#654321]/70'}`}>{key}</p>
        ...
        <div className={`${getCardStyle('purple')} text-center py-8`}>
          ...
          <h3 className={`text-lg font-bold ${theme === 'dark' ? 'text-white' : 'text-[#4B3621]'}`}>Add More Transactions</h3>
          ...
          <div className="flex items-center justify-between mb-4">
            <h3 className={`font-bold text-lg flex items-center gap-2 ${theme === 'dark' ? 'text-white' : 'text-[#4B3621]'}`}>
              ...
              <span className={`text-xs font-bold px-2 py-1 rounded-full ml-1 ${
                theme === 'dark' ? 'bg-blue-500/20 text-blue-400' : 'bg-[#F5F5DC] text-[#654321]'
              }`}>
      ...
      {cashflow && cashflow.timeline && cashflow.timeline.length > 0 ? (
        <div className={getCardStyle('purple')}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className={`p-5 rounded-xl ${theme === 'dark' ? 'bg-white/5 border border-white/10' : 'bg-[#F5F5DC] border border-[#654321]/20'}`}>
              <p className={`text-xs font-bold uppercase mb-2 ${theme === 'dark' ? 'text-gray-400' : 'text-[#654321]/70'}`}>Daily Avg Spending</p>
              <p className={`text-2xl font-black ${theme === 'dark' ? 'text-white' : 'text-[#4B3621]'}`}>
                {currency}{formatIndianNumber(cashflow.predictions.dailyAvgSpending)}
              </p>
            </div>
            <div className={`p-5 rounded-xl ${theme === 'dark' ? 'bg-white/5 border border-white/10' : 'bg-[#F5F5DC] border border-[#654321]/20'}`}>
              <p className={`text-xs font-bold uppercase mb-2 ${theme === 'dark' ? 'text-gray-400' : 'text-[#654321]/70'}`}>Days Remaining</p>
              <p className={`text-2xl font-black ${theme === 'dark' ? 'text-white' : 'text-[#4B3621]'}`}>
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={cashflow.timeline}>
                <CartesianGrid strokeDasharray="3 3" stroke={theme === 'dark' ? '#1e3a8a' : '#D2B48C'} strokeOpacity={0.3} />
                <XAxis dataKey="label" stroke={theme === 'dark' ? '#6b7280' : '#654321'} style={{ fontSize: '12px' }} />
                <YAxis stroke={theme === 'dark' ? '#6b7280' : '#654321'} style={{ fontSize: '12px' }} />
                <Tooltip
                  contentStyle={{
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {budgetIntelligence && budgetIntelligence.budgets && budgetIntelligence.budgets.length > 0 ? (
          <div className={getCardStyle('blue')}>
            ...
            <div className="space-y-4">
              {budgetIntelligence.budgets.slice(0, 5).map((budget, i) => (
                <div key={i} className={`p-4 rounded-xl ${theme === 'dark' ? 'bg-white/5 border border-white/10' : 'bg-[#F5F5DC] border border-[#654321]/20'}`}>
                  <div className="flex justify-between items-start mb-3">
                    <p className={`font-bold text-base ${theme === 'dark' ? 'text-white' : 'text-[#4B3621]'}`}>{budget.category}</p>
                    <span className={`text-xs font-bold px-3 py-1 rounded-full ${
                      budget.status === 'exceeded' || budget.status === 'danger' ? 'bg-red-500/20 text-red-500' :
                      budget.status === 'warning' ? 'bg-yellow-500/20 text-yellow-500' :
                  ...
                  <div className={`w-full h-3 rounded-full overflow-hidden mb-3 ${theme === 'dark' ? 'bg-white/10' : 'bg-white'}`}>
                    <div
                      className={`h-full transition-all duration-1000 ${
                        budget.status === 'exceeded' || budget.status === 'danger' ? 'bg-red-500' :
                        budget.status === 'warning' ? 'bg-yellow-500' : 'bg-blue-500'
                      }`}
                      style={{ width: `${Math.min(budget.percentage, 100)}%` }}
                    />
                  </div>
                  <p className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-[#654321]/70'}`}>{budget.message}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className={`${getCardStyle('blue')} text-center py-8`}>
            ...
            <h3 className={`text-lg font-bold ${theme === 'dark' ? 'text-white' : 'text-[#4B3621]'}`}>Create Budgets</h3>
            <p className={`text-sm mt-2 ${theme === 'dark' ? 'text-gray-400' : 'text-[#654321]/70'}`}>Set up budgets to get intelligent spending insights</p>
          </div>
        )}

        {patterns && patterns.categoryComparison && patterns.categoryComparison.length > 0 ? (
          <div className={getCardStyle('blue')}>
            <h3 className={`font-bold text-lg mb-4 ${theme === 'dark' ? 'text-white' : 'text-[#4B3621]'}`}>SPENDING TRENDS</h3>
            <div className="space-y-3">
              {patterns.categoryComparison.slice(0, 6).map((cat, i) => (
                <div key={i} className={`p-4 rounded-xl ${theme === 'dark' ? 'bg-white/5 border border-white/10' : 'bg-[#F5F5DC] border border-[#654321]/20'}`}>
                  <div className="flex justify-between items-center mb-3">
                    <p className={`font-bold text-sm ${theme === 'dark' ? 'text-white' : 'text-[#4B3621]'}`}>{cat.category}</p>
                    <div className="flex items-center gap-1">
                      ...
                      <span className={`text-sm font-bold ${
                        cat.changeVsLast > 10 ? 'text-red-500' :
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    <div>
                      ...
                      <p className={`font-bold ${theme === 'dark' ? 'text-white' : 'text-[#4B3621]'}`}>{currency}{formatIndianNumber(cat.currentMonth)}</p>
                    </div>
                    <div>
                      <p className={theme === 'dark' ? 'text-gray-500' : 'text-[#654321]/60'}>Last</p>
                      <p className={`font-bold ${theme === 'dark' ? 'text-white' : 'text-[#4B3621]'}`}>{currency}{formatIndianNumber(cat.lastMonth)}</p>
                    </div>
                    <div>
                      <p className={theme === 'dark' ? 'text-gray-500' : 'text-[#654321]/60'}>Avg</p>
                      <p className={`font-bold ${theme === 'dark' ? 'text-white' : 'text-[#4B3621]'}`}>{currency}{formatIndianNumber(cat.threeMonthAvg)}</p>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className={`${getCardStyle('blue')} text-center py-8`}>
            <Info className={`w-12 h-12 mx-auto mb-3 ${theme === 'dark' ? 'text-blue-500' : 'text-[#4B3621]'}`} />
            <h3 className={`text-lg font-bold ${theme === 'dark' ? 'text-white' : 'text-[#4B3621]'}`}>Track Spending</h3>
            <p className={`text-sm mt-2 ${theme === 'dark' ? 'text-gray-400' : 'text-[#654321]/70'}`}>Add more transactions to see spending trends</p>
          </div>
        )}
      </div>
      ...
      <div className={getCardStyle('blue')}>
        <div className="flex justify-between items-center mb-4">
          <h3 className={`font-bold text-lg ${theme === 'dark' ? 'text-white' : 'text-[#4B3621]'}`}>RECENT TRANSACTIONS</h3>
          <button

    ...
    <div className={`flex-1 h-full overflow-y-auto custom-scrollbar animate-fade-in ${styles.bg}`} onClick={() => setShowCalendar(false)}>
      <div className="p-6 flex flex-col gap-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shrink-0 pt-10 md:pt-0">
          ...
          <div className="flex flex-col gap-3 w-full md:flex-row md:w-auto">
            ...
            <div className="flex gap-3 w-full md:w-auto">
              {/* THIS MONTH */}
              <div className={`flex-1 text-right px-4 py-3 rounded-xl border transition duration-500 ${theme === 'dark' ? 'bg-black border-red-500 shadow-[0_0_20px_rgba(239,68,68,0.4)]' : 'bg-[#F5F5DC] border-[#654321]/30 shadow-lg'}`}>
                <p className={`text-[10px] uppercase font-bold tracking-widest ${theme === 'dark' ? 'text-red-400 drop-shadow-[0_0_5px_rgba(239,68,68,0.8)]' : 'text-[#654321]/70'}`}>This Month</p>
                <h3 className={`text-lg md:text-2xl font-bold flex items-center justify-end gap-0.5 leading-tight ${theme === 'dark' ? 'text-white drop-shadow-[0_0_10px_#ef4444]' : 'text-[#4B3621]'}`}>
                  <span className="text-base md:text-xl">-</span>
                  <span>{currency}{formatIndianNumber(currentMonthExpense)}</span>
                </h3>
                {monthChange !== null && (
                  <p className={`text-[10px] font-bold mt-0.5 flex items-center justify-end gap-1 ${parseFloat(monthChange) <= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                    {parseFloat(monthChange) <= 0 ? <ArrowDown size={10} /> : <ArrowUp size={10} />}
            ...
            <button
              ...
              className={`w-full md:w-auto flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-bold transition-all duration-300 hover:scale-105 border ${theme === 'dark' ? 'bg-red-600 text-white shadow-[0_0_20px_#ef4444] hover:shadow-[0_0_40px_#ef4444] border-red-400' : 'bg-[#F5F5DC] text-[#4B3621] border-[#654321]/30 shadow-lg hover:bg-[#F5F5DC]/80'}`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 shrink-0 md:h-72">
          <div className={`${getCardStyle()} col-span-1 md:col-span-2 group min-h-[280px]`}>
            <div className="flex justify-between items-start mb-4">
              <h3 className={`font-bold flex items-center gap-2 text-sm tracking-wide ${theme === 'dark' ? 'text-white drop-shadow-[0_0_5px_rgba(255,255,255,0.3)]' : 'text-[#4B3621]'}`}>
                <DollarSign size={16} className={`${theme === 'dark' ? 'text-red-400 drop-shadow-[0_0_10px_#ef4444]' : 'text-[#4B3621]'}`} /> SPENDING TREND
              </h3>
              <div className={`flex rounded-lg p-1 border ${theme === 'dark' ? 'bg-black border-red-500/50 shadow-[0_0_15px_rgba(239,68,68,0.2)]' : 'bg-white border-[#C9A87C]/50'}`}>
                {['Weekly', 'Monthly', 'Yearly'].map(period => (
                  <button key={period} onClick={() => setChartPeriod(period)} className={`px-3 py-1 text-[10px] rounded-md transition-all font-bold tracking-wider ${chartPeriod === period ? (theme === 'dark' ? 'bg-red-600 text-white shadow-[0_0_15px_#ef4444]' : 'bg-[#F5F5DC] text-[#4B3621] border border-[#654321]/30') : (theme === 'dark' ? 'text-gray-400 hover:text-red-400' : 'text-[#654321]/60 hover:text-[#4B3621]')}`}>{period}</button>
                ))}
              </div>
            </div>
                ...
                <AreaChart data={chartData}>
                  <defs>
                    <linearGradient id="colorExpense" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor={theme === 'dark' ? "#ef4444" : "#654321"} stopOpacity={theme === 'dark' ? 0.8 : 0.4} />
                      <stop offset="95%" stopColor={theme === 'dark' ? "#ef4444" : "#654321"} stopOpacity={0} />
                    </linearGradient>
                  </defs>
          ...
          <div className={`${getCardStyle()} min-h-[200px]`}>
            <h3 className={`font-bold mb-6 text-sm tracking-wide flex items-center gap-2 ${theme === 'dark' ? 'text-white' : 'text-[#4B3621]'}`}>
              <div className={`w-2 h-2 rounded-full ${theme === 'dark' ? 'bg-red-500 shadow-[0_0_10px_#ef4444]' : 'bg-[#654321]'}`}></div>TOP CATEGORIES
            </h3>
        ...
        <div className={`${getCardStyle(true)} !p-0`}>
          <div className={`p-5 border-b flex flex-wrap justify-between items-center gap-3 ${theme === 'dark' ? 'border-red-500/30 bg-red-900/5' : 'border-[#C9A87C]/30 bg-[#F5F5DC]/30'}`}>
            ...
            <div className="flex flex-wrap flex-1 justify-end gap-3">
              <div className={`flex items-center rounded-xl px-4 py-2 w-full sm:w-56 transition border ${theme === 'dark' ? 'bg-black border-red-500/50 focus-within:border-red-400 focus-within:shadow-[0_0_15px_#ef4444]' : 'bg-white border-[#C9A87C]/50 focus-within:border-[#654321]'}`}>
              ...
              <div className="relative" onClick={(e) => e.stopPropagation()}>
                ...
                {showCalendar && (
                  <div className={`absolute top-full right-0 mt-2 z-[100] shadow-2xl rounded-xl border ${theme === 'dark' ? 'border-red-500/50 bg-black shadow-[0_0_30px_rgba(239,68,68,0.2)]' : 'bg-[#FFF8F0] border-[#654321]/30 shadow-lg'}`}>
              ...
              <div key={t.id || t._id || index} className={`grid grid-cols-12 px-6 py-4 border-b transition items-center group ${theme === 'dark' ? 'border-red-500/10 hover:bg-red-500/5' : 'border-[#C9A87C]/20 hover:bg-[#F5F5DC]/30'}`}>
                ...
                <div className={`col-span-5 md:col-span-4 font-bold truncate pr-4 flex items-center gap-3 text-sm ${theme === 'dark' ? 'text-gray-200' : 'text-[#4B3621]'}`}>
                  ...
                  {t.recurrence && t.recurrence !== 'None' && (
                    <span className={`text-[9px] px-2 py-0.5 rounded border tracking-wider ${theme === 'dark' ? 'bg-black text-red-400 border-red-500/50 shadow-[0_0_10px_rgba(239,68,68,0.3)]' : 'bg-[#F5F5DC] text-[#4B3621] border-[#654321]/30'}`}>{t.recurrence}</span>
                  )}
                </div>
                ...
                {/* Date — hidden mobile */}
                <div className={`hidden md:block col-span-2 text-xs font-mono transition ${theme === 'dark' ? 'text-gray-500 group-hover:text-red-300' : 'text-[#654321]/70'}`}>
                  {formatDate(t.date)}
                </div>
                {/* Amount */}
                <div className={`col-span-5 md:col-span-2 font-bold text-sm text-right pr-4 ${theme === 'dark' ? 'text-red-400 drop-shadow-[0_0_5px_rgba(239,68,68,0.6)]' : 'text-[#4B3621]'}`}>
                ...
                <div className="col-span-2 flex justify-center gap-2">
                  <button onClick={() => handleEdit(t, 'expense')} className={`p-1.5 rounded-lg transition border ${theme === 'dark' ? 'bg-black border-red-500/30 text-red-500 hover:bg-red-500 hover:text-black shadow-[0_0_10px_rgba(239,68,68,0.3)]' : 'bg-white text-[#654321] border-[#654321]/30 hover:bg-[#F5F5DC]'}`}>
                    <Edit2 size={14} />
                  </button>

      ...
      <div className={`${getCardStyle(true)} !p-0`}>
         ...
         <div className="overflow-visible">
             {processedData.map((t, index) => (
                 ...
                 <div key={t.id || t._id || index} className={`grid grid-cols-12 px-6 py-4 border-b transition items-center group ${theme === 'neon' ? 'border-red-500/10 hover:bg-red-500/5' : 'border-white/5 hover:bg-white/5'}`}>
                     ...
                     <div className={`col-span-2 text-right font-bold text-red-400 flex justify-end items-center gap-3 text-sm ${theme === 'neon' ? 'drop-shadow-[0_0_5px_rgba(239,68,68,0.6)]' : ''}`}>
                         ...
                         <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0">
                            {/* Updated Edit/Delete buttons to match Red Theme consistency */}
                            <button onClick={() => handleEdit(t, 'expense')} className={`p-1.5 rounded-lg transition ${theme === 'neon' ? 'bg-black border border-red-500/30 text-red-500 hover:bg-red-500 hover:text-black shadow-[0_0_10px_rgba(239,68,68,0.3)]' : 'bg-white/10 text-gray-400 hover:text-white'}`}><Edit2 size={14}/></button>
